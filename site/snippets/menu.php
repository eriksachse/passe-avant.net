  ...
  <div class="fixed w-full z-10 top-0 text-17 ">
    <nav class="flex md:flex-row flex-col text-center justify-center uppercase font-Pbold m-2">
      ...
      <form class="flex-shrink-0 hidden md:block md:mx-4 w-20<?php e($page->template() == "home", ' text-green') ?>" action="<?= page('search')->url() ?>">
        <input type="search" name="q" value="" placeholder="SEARCH" class=" <?php e($page->template() != "home", ' placeholder-black') ?> <?php e($page->template() == "home", ' placeholder-green') ?> uppercase orange-shadow bg-transparent focus:outline-none" onfocus="this.placeholder = ''" onblur="this.placeholder = 'SEARCH'">
      </form>
    </nav>
  </div>
<?php endif ?>